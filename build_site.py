...
def add_publications(file):
    ...
    for regex in [r'\[mono:(.*?)\]',r'\[pub:(.*?)\]',r'\[ipub:(.*?)\]',r'\[talk:(.*?)\]']:
        ...
        for k,pub in enumerate(pubs):            
            for bib_entry in bib_database:
                if bib_entry['ID'] == pub:
                    ...
                    html_new = html_new.replace(f'[mono:{pub}]',pub_html)
...
#build_html('applications/index')
#build_html('intros/index')
